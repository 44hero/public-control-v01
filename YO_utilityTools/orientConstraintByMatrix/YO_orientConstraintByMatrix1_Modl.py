...
.. note::
  jointOrient考慮は、
    ここ
        orientConstraintByMatrix package でカバーしています
...
-リマインダ-
        ...
        - 新規2
            ...
            - 詳細箇所:
                ::
                  ...
                  def search_multMatrixNode_Rotate(self, selectionNode):
                    ...
        version = '-2.1-'

    done: 2023/11/16
        - 生かしている箇所1
            - 概要: orientConstraint だけに特化した記述
            - 詳細箇所:
                ::
    ...
    done: 2023/11/06~2023/11/10
        - 新規1
            - 概要: 新規に作成された utilityNode を一括で確実にリスト登録する関数
            - 詳細箇所:
                ::

                  def check_utilityNode_Connections_Rotate(self, selectionNode):
                    ...
            ...
            - 詳細箇所:
                ::
                  ...
                  def search_decomposeMatrixNode_Rotate(self, selectionNode):
                    ...

        - 変更箇所1
            - 概要: orientConstraint だけに特化した記述に変更
            - 詳細箇所: これまで T, R, S, Shear を一遍に実行していた箇所を R 実行のみにした
                ::

                  def createInitAttr_Rotate(self, tgt, *args):
                    ...
                  def setInitAttr_Rotate(self, tgt, *args):
                    ...
                  def delInitAttr_Rotate(self, tgt, *args):
                    ...
        version = '-1.0-'
...
class OConByMat_Modl(object):
    ...
    def breakAndReset(self, *args):
        ...
        if not sels:
            ...
            dCMtx = self.search_decomposeMatrixNode_Rotate(sels[0])
            ...
            if not dCMtx:
                ...
                if not exist_initR:
                    ...
                    mMtx = self.search_multMatrixNode_Rotate(dCMtx)
                    # 追加11 ####################################################### start
                    ...
                    self.newNodesAll = self.check_utilityNode_Connections_Rotate(sels[0])
                    # 追加11 ####################################################### start
                    ...
                    self.delInitAttr_Rotate(sels[0])
                    ...
                    message(u'rotate '
                            ...
                            u'また、当ツールを使用した、'
                            u'orientConstraintByMatrix を実行する前の、ローカル数値への'
                            u'リセットも行いました。'
                            u'ご確認ください。'
                            )
                    # 修正箇所9 ########################################### end
    ...
    def command(self, *args, **kwargs):
        ...
        if len(src) and len(tgt):  # src, tgt 共に入力セッティングされていれば、実行
            if connectAll:  # 全ての接続を実行
                ...
                utilNodeShortNameSet = 'mult Mtrx' + 'Rote'  # 変更箇所1
                ...
                utilNodeShortNameSet = 'dcmp Mtrx' + 'Rote'  # 変更箇所1
                self.strCompLists_count_caseDividing_exe(strsCompLists
                                                         , utilNodeShortNameSet
                                                         )  # naming
                ...
                if cmds.nodeType(tgt) == 'joint':  # tgt が joint の場合
                    ...
                    utilNodeShortNameSet = 'elerTo Quat' + 'Rote'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quat Invt' + 'Rote'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quat Prod' + 'Rote'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quatTo Eler' + 'Rote'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    # 追加11 ####################################################### start
                    self.scriptEditor2.append_default('***' * 15)
                    self.scriptEditor2.append_default('outPutNode : {}, inPutNode : {}'.format(qTEr, tgt))
                    # self.scriptEditor2.append_default(self.outPutNode, self.inPutNode)
                    self.scriptEditor2.append_default('***' * 15)
                    self.scriptEditor2.append_default('Rotate connected..')
                    # 追加11 ####################################################### end
                    # print('***' * 15)
                ...
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default('***' * 15)
                self.scriptEditor2.append_default(u'Rotate connected..done')
                # 追加11 ####################################################### end
                # print('***' * 15)
                ...
                self.createInitAttr_Rotate(tgt)  # add user defined attr
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default('***' * 15 + '\n')
                self.scriptEditor2.append_default('***' * 15)
    ...
    def check_utilityNode_Connections_Rotate(self, selectionNode):
        ...
        #.
            :param str selectionNode : rename mode: 強制的:0, 構成要素をキープ:1 何れか
        ...
        for eTQt_index in eTQt:
            ...
            if qInv[0] and 'Rote' in qInv[0]:  # qInv[0] is True 且つ、 文字列'Rote'を含む
                qPrd = cmds.listConnections(qInv[0]
                                            , et = True
                                            , t = 'quatProd'
                                            ) or []
                # check_utilityNodeAll.append(qPrd[0])
                if qPrd[0] and 'Rote' in qPrd[0]:  # qPrd[0] is True 且つ、 文字列'Rote'を含む
                    qTEr = cmds.listConnections(qPrd[0]
                                                , et = True
                                                , t = 'quatToEuler'
                                                ) or []
                    # check_utilityNodeAll.append(qTEr[0])
                    if qTEr[0] and 'Rote' in qTEr[0]:  # qTEr[0] is True 且つ、 文字列'Rote'を含む
                        ...
                        # if not connectList:
                        #     pass
                        # if connectList:  # コネクションがされていれば 真 で実行
                        ...
                        if not any(isConnectedRxyz):  # 3つの要素の内少なくとも1つがFalseである場合 真 で実行
                            pass
                        elif not all(x for x in isConnectedRxyz):  # 3つの要素全てがFalseである場合 真 で実行
                            pass
                        # [True, True, True]
                        else:  # 3つの要素全てがTrue、つまり全てコネクションがされていたら 真 で実行
                            check_utilityNodeAll.append(eTQt_index)
                            check_utilityNodeAll.append(qInv[0])
                            check_utilityNodeAll.append(qPrd[0])
                            check_utilityNodeAll.append(qTEr[0])
    ...
    def search_decomposeMatrixNode_Rotate(self, selectionNode):
        ...
        # quatToEuler を確実に探し当てるために... ############################ start
        # qTEr = cmds.listConnections(selectionNode
        #                             , et = True
        #                             , t = 'quatToEuler'
        #                             ) or []
        # # check_utilityNodeAll.append(qTEr[0])
        qTErX = cmds.listConnections('{}.rx'.format(selectionNode)
                                     , et = True
                                     , t = 'quatToEuler'
                                     ) or []
        qTErY = cmds.listConnections('{}.ry'.format(selectionNode)
                                     , et = True
                                     , t = 'quatToEuler'
                                     ) or []
        qTErZ = cmds.listConnections('{}.rz'.format(selectionNode)
                                     , et = True
                                     , t = 'quatToEuler'
                                     ) or []
        # quatToEuler を確実に探し当てるために... ############################## end

        # decomposeMatrix を確実に探し当てるために... ######################## start
        if not any(qTErX and qTErY and qTErZ):  # 3つの要素の内少なくとも1つがFalseである場合 真 で実行
            pass
            # 追加11 ####################################################### start
            self.scriptEditor2.append_default('any or all None')
            # 追加11 ####################################################### end
            # print('any or all None')
        elif qTErX[0] and qTErY[0] and qTErZ[0]:  # 3つの要素全てがTrue、つまりコネクションがされていたら 真 で実行
            qPrd = cmds.listConnections(qTErX[0]
                                        , et = True
                                        , t = 'quatProd'
                                        ) or []
            # check_utilityNodeAll.append(qPrd[0])
            if qPrd[0]:
                dCMtx = cmds.listConnections(qPrd[0]
                                             , et = True
                                             , t = 'decomposeMatrix'
                                             ) or []
                dCMtxNode = dCMtx[0]
    ...
    def search_multMatrixNode_Rotate(self, selectionNode):
        ...
        if mMtx[0] and 'Rote' in mMtx[0]:  # mMtx[0] is True 且つ、 文字列'Trans'を含む
            mMtxNode = mMtx[0]
        return mMtxNode
