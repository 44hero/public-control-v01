...
# print(fileName)  # YO_renameTool5_main.py
# print(directory)  # renameTool
# print(parentDir)  # YO_utilityTools
#
# 基本的にはモジュール実行ベースの記述だが、
# PyCharm等のデバッグポート使用時のスクリプト実行も可能にするため、あえてパッケージ宣言
__package__ = f'{parentDir}.{directory}'  # "YO_utilityTools.renameTool"
# あえてパッケージ宣言  ####################### end

# mvc_model_module_directly_related_to_itself(自身に直接関わるMVCモデルモジュール)
from .YO_renameTool5_Modl import RT_Modl
from .YO_renameTool5_View import RT_View
from .YO_renameTool5_Ctlr import RT_Ctlr
...
if __name__ == '__main__':
    ...
    # for key in YO_utilityTools.renameTool.__dict__:
    #     if not key.startswith('__'):
    #         # print(YO_utilityTools.renameTool.__dict__[key].__name__)
    #         if key.endswith('_main'):
    #             fileName = YO_utilityTools.renameTool.__dict__[key].__name__
    #             # print(fileName)
    #
    #             print('-' * 100)
    #             print(u'{}.py: loaded as script file'.format(fileName))
    #             filePath = YO_utilityTools.renameTool.__dict__[key].__file__
    #             # print(filePath)
    #             print('{}'.format(filePath))
    #             # print_loaded_modules(filePath)  # 開発時(development)のみコメントアウトの除外を推奨
    #             print('-' * 100)
