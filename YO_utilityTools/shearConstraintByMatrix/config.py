...
u"""
shearConstraintByMatrix package
...
概要(overview):
    matrix を使用した、shear constraint を自動作成するツールです。
詳細(details):
    新規に、shear 拘束処理を matrix 操作で実現します。
...
使用法(usage):
    ::
        ...
        import YO_utilityTools.shearConstraintByMatrix.YO_shearConstraintByMatrix1_main
        reload(YO_utilityTools.shearConstraintByMatrix.YO_shearConstraintByMatrix1_main)
    ...
    <接続編>
        ::
            ...
            # 以下 e.g.):
            # import <パッケージ名>.<モジュール名>
            import YO_utilityTools.shearConstraintByMatrix.YO_shearConstraintByMatrix1_Modl
            reload(YO_utilityTools.shearConstraintByMatrix.YO_shearConstraintByMatrix1_Modl)
            # from <パッケージ名> import <モジュール名>
            from YO_utilityTools.shearConstraintByMatrix import YO_shearConstraintByMatrix1_Modl
            YO_shearConstraintByMatrix1_Modl.ShConByMat_Modl().command(connect = True, source = 'spineC', target = 'spineC_jtPxy')
    ...
    <切断編>
        ::
            ...
            # from <パッケージ名> import <モジュール名>
            from YO_utilityTools.shearConstraintByMatrix import YO_shearConstraintByMatrix1_Modl
            YO_shearConstraintByMatrix1_Modl.ShConByMat_Modl().brkAndRest_command(target = 'spineC_jtPxy')
        ...
        使用法(usage):
            ...
            <longName>:
                ::
                    ...
                    YO_shearConstraintByMatrix1_Modl.ShConByMat_Modl().command(connect = True
                    , source = 'spineC'
                    , target = 'spineC_jtPxy'
                    )
            <shortName>:
                ::
                    ...
                    YO_shearConstraintByMatrix1_Modl.ShConByMat_Modl().command(ct = True
                    , src = 'spineC'
                    , tgt = 'spineC_jtPxy'
                    )
    <切断編>
        使用法(usage):
            ...
            <shortName>:
                ::
                    ...
                    YO_shearConstraintByMatrix1_Modl.ShConByMat_Modl().brkAndRest_command(tgt = 'spineC_jtPxy')
...
-リマインダ-
        ...
        - 変更12
            - 概要: source, target について、
                ...
                以降の実行中止させる 明示と中止の実行
                - YO_shearConstraintByMatrix1_Modl.py 変更記述あり
            詳細はそれ自身のリマインダに記述...
        version = '-5.5-'
        ...
        - 修正11
            - 概要: maya default の Script Editor へのログ出力 を、
                ...
                - YO_shearConstraintByMatrix1_Modl.py 変更記述あり
                - YO_shearConstraintByMatrix1_View.py 変更記述あり
            詳細はそれ自身のリマインダに記述...
        version = '-5.0-'
        ...
        - 変換箇所8
            絶対パス から 相対パス へ記述変更
                - YO_shearConstraintByMatrix1_Modl.py 変更記述あり
                - YO_shearConstraintByMatrix1_View.py 変更記述あり
                - YO_shearConstraintByMatrix1_Ctlr.py 変更記述あり
                - YO_shearConstraintByMatrix1_main.py 変更記述あり
            詳細はそれ自身のリマインダに記述...
    ...
    done: 2023/11/09~2023/11/15
        新規

        version = '-1.0-'
"""

TITLE = 'YO_shearConstraintByMatrix1'  # MVCモデルを意識しファイル分割した、パッケージ記述
