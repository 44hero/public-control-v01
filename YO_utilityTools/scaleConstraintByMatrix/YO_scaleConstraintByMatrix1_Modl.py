...
-リマインダ-
        ...
        - 新規3
            - 概要: scaleConstraintByMatrix 限定で、
                inverseMatrix 接続の存在 と inverseScale 接続の存在 が、
                    ダブルで実行されている個所があるため、
                        その箇所だけ、その inverseMatrix 接続 を切断する必要がある
            - 詳細箇所:
                コードを追加
        ...
        - 新規2
            ...
            - 詳細箇所:
                ::
                  ...
                  def search_multMatrixNode_Scal(self, selectionNode):
                    ...
        ...
        - 新規1
            ...
            - 詳細箇所:
                ::
                  ...
                  def check_utilityNode_Connections_Scal(self, selectionNode):
                    ...
            ...
            - 詳細箇所:
                ::
                  ...
                  def search_decomposeMatrixNode_Scal(self, selectionNode):
                    ...

        - 変更箇所1
            - 概要: scaleConstraint だけに特化した記述に変更
            - 詳細箇所: これまで T, R, S, Shear を一遍に実行していた箇所を S 実行のみにした
                ::

                  def createInitAttr_Scal(self, tgt, *args):
                    ...
                  def setInitAttr_Scal(self, tgt, *args):
                    ...
                  def delInitAttr_Scal(self, tgt, *args):
                    ...
...
class SConByMat_Modl(object):
    ...
    # 新規作成された outPutNode と inPutNode のコネクションを行う
    # (Scale 限定)
    def oP2iP_cnctS_exe(self, outPutNode, inPutNode):
        u""" <新規作成された outPutNode と inPutNode のコネクションを行う
        (Scale 限定)
        >
        """
    ...
    def main_followTheHierarchyAndConnection_exe(self):
        ...
        # 追加11 ####################################################### start
        self.scriptEditor2.append_default(selList)  # type : list of 'unicode'
        # 追加11 ####################################################### end
        ...
        if len(selList) == 0 or len(selList) == 1 or len(selList) >= 3:
            ...
            if self.followHierarchy_isConnected_ToF == 0:  # 未だ接続されていないので、接続
                ...
                # print(connected_counter)  # type: list[int]
                # print(connected_counter[-1])  # type: int # end of index, src to mM
                self.notYetConnected_counterStart = connected_counter[-1] + 1
                # print(self.notYetConnected_counterStart)  # type: int # start of index, tgt to mM
                # srcList_temp を階層下から上に順に接続 ######################## end

                # tgtList_temp を階層上から下へ順に接続 ###################### start
                for i, tgt in enumerate(tgtList_temp
                        , start = self.notYetConnected_counterStart):  # start of index, tgt to mM
                    ...
                    if ToF:
                        ...
                        connected_counter.append(i)
                # print(connected_counter)  # type: list[int]
                # print(connected_counter[-1])  # type: int # end of index, tgt to mM
                self.notYetConnected_counterEnd = connected_counter[-1] + 1  # note:) 存在しません!
                # print(self.notYetConnected_counterEnd)
                # tgtList_temp を階層下から上に順に接続 ######################## end

                # 追加11 ####################################################### start
                self.scriptEditor2.append_default(u'一旦 connected all  !!!')
                # 追加11 ####################################################### end
                # print(u'一旦 connected all  !!!')

                # 新規3
                # scaleConstraintByMatrix 限定 ダブルinverseScale ########## start
                # 次に、scaleConstraintByMatrix 限定で、
                # inverseMatrix 接続の存在 と inverseScale 接続の存在 が、
                # ダブルで実行されている個所があるため、
                # その箇所だけ、その inverseMatrix 接続 を切断する必要がある
                startOfIndexTgtToMMatrix = self.notYetConnected_counterStart
                soiTgt2mM = startOfIndexTgtToMMatrix
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default(soiTgt2mM)
                # 追加11 ####################################################### end
                # print(soiTgt2mM)

                i = 1
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default(len(tgtList_temp))
                self.scriptEditor2.append_default(tgtList_temp)
                # 追加11 ####################################################### end
                # print(len(tgtList_temp))
                # print(tgtList_temp)

                while i < len(tgtList_temp) :
                    # 以下、proc A. - > proc B. と辿ります ################## start

                    # proc A.
                    # tgtList_temp[i - 1] と tgtList_temp[i] の、
                    # .scale と .inverseScale の接続を調べ、
                    # 接続されていたら、

                    # proc B.
                    # tgtList_temp[i] と mM[{soiTgt2mM + i}] の、
                    # .inverseMatrix と matrixIn の接続を切断し、解除する

                    # proc A.
                    # 追加11 ####################################################### start
                    self.scriptEditor2.append_default('--------------' * 2)
                    # 追加11 ####################################################### end
                    # print('--------------' * 2)

                    # print(tgtList_temp[i - 1])
                    # print(tgtList_temp[i])
                    isConnected_ = cmds.isConnected(f'{tgtList_temp[i - 1]}.scale'
                                                    , f'{tgtList_temp[i]}.inverseScale'
                                                    )
                    # print(isConnect)
                    ...
                    # print(f'{tgtList_temp[i]}.inverseMatrix')
                    # print(f'{mM}.matrixIn' + f'[{soiTgt2mM + i}]')

                    if isConnected_:
                        cmds.disconnectAttr(f'{tgtList_temp[i]}.inverseMatrix'
                                            , f'{mM}.matrixIn' + f'[{soiTgt2mM + i}]'
                                            )
                        # 追加11 ####################################################### start
                        self.scriptEditor2.append_default(u'部分的に disconnect 実行完了...!!')
                        # 追加11 ####################################################### end
                        # print(u'部分的に disconnect 実行完了...!!')
                    ...
                    i += 1
                    # 以下、proc A. - > proc B. と辿ります #################### end
                # scaleConstraintByMatrix 限定 ダブルinverseScale ############ end
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default(u'一部 disconnect 実行して connect 完了 !!!')
                # 追加11 ####################################################### end
                # print(u'一部 disconnect 実行して connect 完了 !!!')
    ...
    def breakAndReset(self, *args):
        ...
        if not sels:
            ...
            dCMtx = self.search_decomposeMatrixNode_Scal(sels[0])
            ...
            if not dCMtx:
                ...
                if not exist_initS:
                    ...
                    mMtx = self.search_multMatrixNode_Scal(dCMtx)
                    # 追加11 ####################################################### start
                    ...
                    self.newNodesAll = self.check_utilityNode_Connections_Scal(sels[0])
                    # 追加11 ####################################################### start
                    ...
                    message(u'scale '
                            ...
                            u'また、当ツールを使用した、'
                            u'scaleConstraintByMatrix を実行する前の、ローカル数値への'
                            u'リセットも行いました。'
                            u'ご確認ください。'
                            )
                    # 修正箇所9 ########################################### end
    ...
    def command(self, *args, **kwargs):
        ...
        if len(src) and len(tgt):  # src, tgt 共に入力セッティングされていれば、実行
            if connectAll:  # 全ての接続を実行
                ...
                utilNodeShortNameSet = 'mult Mtrx' + 'Scal'  # 変更箇所1
                ...
                utilNodeShortNameSet = 'dcmp Mtrx' + 'Scal'  # 変更箇所1
                self.strCompLists_count_caseDividing_exe(strsCompLists
                                                         , utilNodeShortNameSet
                                                         )  # naming
                ...
                if cmds.nodeType(tgt) == 'joint':  # tgt が joint の場合
                    ...
                    utilNodeShortNameSet = 'elerTo Quat' + 'Scal'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quat Invt' + 'Scal'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quat Prod' + 'Scal'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quatTo Eler' + 'Scal'  # 変更箇所1
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    self.scriptEditor2.append_default('***' * 15)
                    self.scriptEditor2.append_default('Scale connected..')
                    # 追加11 ####################################################### end
                    # print('outPutNode2 : {}, inPutNode : {}'.format(dM, tgt))
                    # print(self.outPutNode2, self.inPutNode)
                    # print('***' * 15)
                ...
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default('***' * 15)
                self.scriptEditor2.append_default(u'Scale connected..done')
                self.scriptEditor2.append_default('***' * 15 + '\n')
                # 追加11 ####################################################### end
                # print('***' * 15)
                # print(u'Scale connected..done')
                # print('***' * 15 + '\n')
                ...
                self.createInitAttr_Scal(tgt)  # add user defined attr
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default('***' * 15 + '\n')
                self.scriptEditor2.append_default('***' * 15)
    ...
    def check_utilityNode_Connections_Scal(self, selectionNode):
        ...
        for eTQt_index in eTQt:
            ...
            if qInv[0] and 'Scal' in qInv[0]:  # qInv[0] is True 且つ、 文字列'Scal'を含む
                qPrd = cmds.listConnections(qInv[0]
                                            , et = True
                                            , t = 'quatProd'
                                            ) or []
                # check_utilityNodeAll.append(qPrd[0])
                if qPrd[0] and 'Scal' in qPrd[0]:  # qPrd[0] is True 且つ、 文字列'Scal'を含む
                    qTEr = cmds.listConnections(qPrd[0]
                                                , et = True
                                                , t = 'quatToEuler'
                                                ) or []
                    # check_utilityNodeAll.append(qTEr[0])
                    if qTEr[0] and 'Scal' in qTEr[0]:  # qTEr[0] is True 且つ、 文字列'Scal'を含む
    ...
    def search_decomposeMatrixNode_Scal(self, selectionNode):
        ...
        elif dCMtxOutSX and dCMtxOutSY and dCMtxOutSZ:  # 6つの要素全てがTrue、つまりコネクションがされていたら 真 で実行
            dCMtxNode = dCMtxOutSX[0]
        return dCMtxNode
    ...
    def search_multMatrixNode_Scal(self, selectionNode):
        ...
        if mMtx[0] and 'Scal' in mMtx[0]:  # mMtx[0] is True 且つ、 文字列'Trans'を含む
            mMtxNode = mMtx[0]
        return mMtxNode
