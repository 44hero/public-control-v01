        ...
        - python2系 -> python3系 変換
            - 変換箇所3
                ...
                ::
                  ...
                  +
                          if __name__ == '__main__':
                            # print(u'{}.py: loaded as script file'.format(__name__))
                            for key in YO_utilityTools.createClusterAndRenameTool.__dict__:
            ...
            - 変換箇所4
                ...
                ::
                  ...
                  +
                          from YO_utilityTools.YO_printLoadedModules import print_loaded_modules
        version = '-2.5-'
    done: 2023/09/21
        - python2系 -> python3系 変換
            - 変換箇所1
...
# import YO_utilityTools.createClusterAndRenameTool  # createClusterAndRenameTool パッケージに対して
# for key in YO_utilityTools.createClusterAndRenameTool.__dict__:
#     if not key.startswith('__'):
#         reload(YO_utilityTools.createClusterAndRenameTool.__dict__[key])
...
# print(fileName)  # YO_createClusterAndRename6_main.py
# print(directory)  # createClusterAndRename
# print(parentDir)  # YO_utilityTools
#
# 基本的にはモジュール実行ベースの記述だが、
# PyCharm等のデバッグポート使用時のスクリプト実行も可能にするため、あえてパッケージ宣言
__package__ = f'{parentDir}.{directory}'  # "YO_utilityTools.createClusterAndRename"
# あえてパッケージ宣言  ####################### end

# mvc_model_module_directly_related_to_itself(自身に直接関わるMVCモデルモジュール)
from .YO_createClusterAndRename6_Modl import CCAndRT_Modl
from .YO_createClusterAndRename6_View import CCAndRT_View
from .YO_createClusterAndRename6_Ctlr import CCAndRT_Ctlr
...
if __name__ == '__main__':
    ...
    # for key in YO_utilityTools.createClusterAndRenameTool.__dict__:
    #     if not key.startswith('__'):
    #         # print(YO_utilityTools.createClusterAndRenameTool.__dict__[key].__name__)
    #         if key.endswith('_main'):
    #             fileName = YO_utilityTools.createClusterAndRenameTool.__dict__[key].__name__
    #             # print(fileName)
    #
    #             print('-' * 100)
    #             print(u'{}.py: loaded as script file'.format(fileName))
    #             filePath = YO_utilityTools.createClusterAndRenameTool.__dict__[key].__file__
    #             # print(filePath)
    #             print('{}'.format(filePath))
    #             # print_loaded_modules(filePath)  # 開発時(development)のみコメントアウトの除外を推奨
    #             print('-' * 100)
