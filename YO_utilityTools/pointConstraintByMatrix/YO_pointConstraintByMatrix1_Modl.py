...
.. note::
  リマインダ-記述内の、
    version = '-3.0-'
        変更箇所2 の内、
            jointOrient考慮した方が良いのか怪しい。。。箇所あり。
                現状は、orientConstraintByMatrix package でカバーしている。
            この該当箇所のコメントアウトを解放することで、
                jointOrient考慮した仕様に変更しますが、
                    後の工程に支障が出ます。
...
-リマインダ-
        ...
        - 新規2
            ...
            - 詳細箇所:
                ::
                  ...
                  def search_multMatrixNode_Trans(self, selectionNode):
                    ...
    ...
    done: 2023/11/03~2023/11/10
        - 新規1
            - 概要: 新規に作成された utilityNode を一括で確実にリスト登録する関数
            - 詳細箇所:
                ::

                  def check_utilityNode_Connections_Trans(self, selectionNode):
                    ...
            ...
            - 詳細箇所:
                ::
                  ...
                  def search_decomposeMatrixNode_Trans(self, selectionNode):
                    ...

        - 変更箇所2
            - 概要: pointConstraint だけに特化した記述に変更
            - 詳細箇所: これまで T, R, S, Shear を一遍に実行していた箇所を T 実行のみにした
                ::

                  def createInitAttr_Trans(self, tgt, *args):
                    ...
                  def setInitAttr_Trans(self, tgt, *args):
                    ...
                  def delInitAttr_Trans(self, tgt, *args):
                    ...
        ...
        - 変更箇所1
            - 概要: 登録・確認用ボタンの、1関数実行実現のため、引数付き1関数を新規に定義
            - 詳細箇所:
                ::
...
class PConByMat_Modl(object):
    ...
    def breakAndReset(self, *args):
        ...
        if not sels:
            ...
            dCMtx = self.search_decomposeMatrixNode_Trans(sels[0])
            ...
            if not dCMtx:
                ...
                if not exist_initT:
                    ...
                    mMtx = self.search_multMatrixNode_Trans(dCMtx)
                    # 追加11 ####################################################### start
                    ...
                    self.newNodesAll = self.check_utilityNode_Connections_Trans(sels[0])
                    ...
                    message(u'translate '
                            ...
                            u'また、当ツールを使用した、'
                            u'pointConstraintByMatrix を実行する前の、ローカル数値への'
                            u'リセットも行いました。'
                            u'ご確認ください。'
                            )
                    # 修正箇所9 ########################################### end
    ...
    def command(self, *args, **kwargs):
        ...
        if len(src) and len(tgt):  # src, tgt 共に入力セッティングされていれば、実行
            if connectAll:  # 全ての接続を実行
                ...
                utilNodeShortNameSet = 'mult Mtrx' + 'Tras'  # 変更箇所2
                ...
                utilNodeShortNameSet = 'dcmp Mtrx' + 'Tras'  # 変更箇所2
                self.strCompLists_count_caseDividing_exe(strsCompLists
                                                         , utilNodeShortNameSet
                                                         )  # naming
                ...
                if cmds.nodeType(tgt) == 'joint':  # tgt が joint の場合
                    ...
                    utilNodeShortNameSet = 'elerTo Quat' + 'Tras'  # 変更箇所2
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quat Invt' + 'Tras'  # 変更箇所2
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quat Prod' + 'Tras'  # 変更箇所2
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    utilNodeShortNameSet = 'quatTo Eler' + 'Tras'  # 変更箇所2
                    self.strCompLists_count_caseDividing_exe(strsCompLists
                                                             , utilNodeShortNameSet
                                                             )  # naming
                    ...
                    self.scriptEditor2.append_default('***' * 15)
                    self.scriptEditor2.append_default('Translate connected..')
                    # 追加11 ####################################################### end
                    # print('outPutNode2 : {}, inPutNode : {}'.format(dM, tgt))
                    # print(self.outPutNode2, self.inPutNode)
                    # print('***' * 15)
                ...
                # 追加11 ####################################################### start
                self.scriptEditor2.append_default('***' * 15)
                self.scriptEditor2.append_default(u'Translate connected..done')
                self.scriptEditor2.append_default('***' * 15 + '\n')
                # 追加11 ####################################################### end
                # print('***' * 15)
                ...
                self.createInitAttr_Trans(tgt)  # add user defined attr
    ...
    def cnctSrcHircyTgtHircy2MltMat_exe(self, *args, **kwargs):
        ...
        if not srcP_list:
            ...
            srcP = self.srcP
            self.followHierarchy_isConnected_ToF = cmds.isConnected(
                '{}.matrix'.format(srcP), '{}.matrixIn[2]'.format(mM)
                )
    ...
    def check_utilityNode_Connections_Trans(self, selectionNode):
        ...
        for eTQt_index in eTQt:
            ...
            if qInv[0] and 'Tras' in qInv[0]:  # qInv[0] is True 且つ、 文字列'Tras'を含む
                qPrd = cmds.listConnections(qInv[0]
                                            , et = True
                                            , t = 'quatProd'
                                            ) or []
                # check_utilityNodeAll.append(qPrd[0])
                if qPrd[0] and 'Tras' in qPrd[0]:  # qPrd[0] is True 且つ、 文字列'Tras'を含む
                    qTEr = cmds.listConnections(qPrd[0]
                                                , et = True
                                                , t = 'quatToEuler'
                                                ) or []
                    # check_utilityNodeAll.append(qTEr[0])
                    if qTEr[0] and 'Tras' in qTEr[0]:  # qTEr[0] is True 且つ、 文字列'Tras'を含む
    ...
    def search_decomposeMatrixNode_Trans(self, selectionNode):
        ...
        #                             ) or []
        # # check_utilityNodeAll.append(qTEr[0])
        dCMtxX = cmds.listConnections('{}.tx'.format(selectionNode)
                                      , et = True
                                      , t = 'decomposeMatrix'
                                      ) or []
        dCMtxY = cmds.listConnections('{}.ty'.format(selectionNode)
                                      , et = True
                                      , t = 'decomposeMatrix'
                                      ) or []
        dCMtxZ = cmds.listConnections('{}.tz'.format(selectionNode)
                                      , et = True
                                      , t = 'decomposeMatrix'
                                      ) or []
        # decomposeMatrix を確実に探し当てるために... ########################## end
        if not any(dCMtxX and dCMtxY and dCMtxZ):  # 3つの要素の内少なくとも1つがFalseである場合 真 で実行
            pass
            # 追加11 ####################################################### start
            self.scriptEditor2.append_default('any or all None')
            # 追加11 ####################################################### end
            # print('any or all None')
        elif dCMtxX[0] and dCMtxY[0] and dCMtxZ[0]:  # 3つの要素全てがTrue、つまりコネクションがされていたら 真 で実行
            dCMtxNode = dCMtxX[0]
        return dCMtxNode
    ...
    def search_multMatrixNode_Trans(self, selectionNode):
        ...
        if mMtx[0] and 'Tras' in mMtx[0]:  # mMtx[0] is True 且つ、 文字列'Tras'を含む
            mMtxNode = mMtx[0]
        return mMtxNode
